...
for seed_iter in xrange(args.seeds):
    ...
    for iters in xrange(args.grad_iterations):
        ...
        if not label1 == label2 == label3:
            ...
            # jteoh note: using decode_label on preds because that's what the other statement (output already differs) does.
            print("Found output which causes difference in models' predictions: %s vs %s vs %s" % (
            decode_label(pred1), decode_label(pred2), decode_label(pred3)))
            print(
